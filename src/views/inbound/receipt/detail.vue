...
async function handleScanReel() {
  ...
  try {
    ...
    if (!targetReel || !targetReel.id) {
      throw new Error(`未找到编号为 ${code} 的线盘信息`)
    }
    ...
    if (!targetDetail) {
      message.error('未找到对应明细，无法执行入库')
      scanInputRef.value?.select()
      return
    }

    if (targetDetail.isReceived) {
      message.warning('该明细已完成收货，无需重复执行')
      scanInputRef.value?.select()
      return
    }
    ...
    if (!craftVersion) {
      message.error('该明细缺少工艺版本，无法按盘收货，请使用单行收货并填写工艺版本')
      scanInputRef.value?.select()
      return
    }

    // 用 planQuantity 作为本次要提交的实收数量，校验不能 <= 0
...
<template>
  <div class="page">
    ...
    <n-card :bordered="false">
      <div class="scanner-bar">
        <n-input
          ...
          size="large"
          placeholder="请扫描托盘号/线盘号 (Enter执行)"
          @keydown.enter="handleScanReel"
        />
        <n-select
          v-model:value="defaultWarehouseId"
          filterable
          remote
          clearable
          ...
          @update:value="handleDefaultWarehouseChange"
        />
        <n-select
          v-model:value="defaultLocationId"
          filterable
          remote
          clearable
    ...
    <!-- Modal -->
    <n-modal v-model:show="modalVisible" preset="card" title="单行收货" style="width: 640px">
      <n-form ref="formRef" :model="executeForm" :rules="rules" label-width="120">
        <n-form-item label="仓库">
          <n-select
            v-model:value="modalWarehouseId"
            filterable
            remote
            clearable
            :loading="warehouseLoading"
            ...
            placeholder="选择仓库"
            @search="handleWarehouseSearch"
            @update:value="handleModalWarehouseChange"
          />
        </n-form-item>

        <n-form-item label="实收数量" path="actualQuantity">
          <n-input-number v-model:value="executeForm.actualQuantity" :min="1" :step="1" placeholder="必须大于0" />
        </n-form-item>

        <n-form-item label="实收重量">
          <n-input-number v-model:value="executeForm.weight" :min="0" :step="0.1" />
        </n-form-item>

        <n-form-item label="上架库位" path="locationId">
          <n-select
            v-model:value="executeForm.locationId"
            filterable
            remote
            clearable
        ...
        <n-form-item label="SN/条码">
          <n-input v-model:value="executeForm.sn" placeholder="可选" />
        </n-form-item>

        <n-form-item label="批次号">
          <n-input v-model:value="executeForm.batchNo" placeholder="可选" />
        </n-form-item>

        <n-form-item label="工艺版本" path="craftVersion">
          <n-input v-model:value="executeForm.craftVersion" placeholder="必填" />
        </n-form-item>

        <n-form-item label="库存状态">
          <n-select
            v-model:value="executeForm.status"
            :options="[
