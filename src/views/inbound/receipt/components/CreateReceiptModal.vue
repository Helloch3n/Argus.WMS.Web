...
async function handleSubmit() {
  ...
  for (const row of details.value) {
    if (!row.productId) {
      message.error('明细中存在未选择产品的行')
      return
    }
    if (!row.productName?.trim()) {
      message.error('明细中存在未填写产品名称的行')
      return
    }
    if (!row.unit?.trim()) {
      message.error('明细中存在未填写单位的行')
      return
    }
    if (!row.planQuantity || row.planQuantity <= 0) {
      message.error('明细中存在计划数量不正确的行')
      return
    }
    if (!row.batchNo?.trim()) {
      message.error('明细中存在未填写批次号的行')
      return
    }
    if (!row.sourceWo?.trim()) {
      message.error('明细中存在未填写来源工单的行')
      return
    }
    if (!row.reelId) {
      message.error('明细中存在未选择线盘的行')
      return
    }
    if (!row.reelNo?.trim()) {
      message.error('明细中存在未填写线盘号的行')
      return
    }
    if (!row.sn?.trim()) {
      message.error('明细中存在未填写 SN/条码 的行')
      return
    }
    if (!row.craftVersion?.trim()) {
      message.error('明细中存在未填写工艺版本的行')
      return
    }
    if (!row.layerIndex || row.layerIndex <= 0) {
      message.error('明细中存在层级不正确的行')
      return
    }
    if (!row.toWarehouseId) {
      message.error('明细中存在未选择目标仓库的行')
      return
    }
...
<template>
  <n-modal v-model:show="visible" preset="card" title="新增入库单" style="width: 100vw; height: 100vh;">
    <n-form :model="form" :rules="headerRules" label-width="100">
      <n-space vertical :size="16">
        <n-card size="small" title="单据头" :bordered="false">
          <n-space :size="16" align="center" wrap>
            <n-form-item label="单据号">
              <div class="form-text">系统自动生成</div>
            </n-form-item>
            <n-form-item label="单据类型" path="type">
              <n-select v-model:value="form.type" :options="typeOptions" style="width: 180px" />
            </n-form-item>
            <n-form-item label="来源单号">
              <n-input v-model:value="form.sourceBillNo" placeholder="来源单号" style="width: 220px" />
            </n-form-item>
        ...
        <n-card size="small" title="明细" :bordered="false">
          <n-dynamic-input v-model:value="details" :on-create="createDetail">
              ...
              <div class="detail-row">
                <div class="detail-field">
                  ...
                  <n-select
                    ...
                    @update:value="(val) => handleProductChange(val, value)"
                  />
                </div>

                <div class="detail-field">
                  <div class="detail-label required">单位</div>
                  <n-input v-model:value="value.unit" placeholder="单位" />
                </div>

                <div class="detail-field">
                  <div class="detail-label required">计划数量</div>
                  <n-input-number v-model:value="value.planQuantity" :min="0" placeholder="计划数量" />
                </div>

                <div class="detail-field">
                  <div class="detail-label required">批次号</div>
                  <n-input v-model:value="value.batchNo" placeholder="批次号" />
                </div>

                <div class="detail-field">
                  <div class="detail-label required">来源工单</div>
                  <n-input v-model:value="value.sourceWo" placeholder="来源工单" />
                </div>

                <div class="detail-field">
                  <div class="detail-label required">线盘</div>
                  <n-select
                    v-model:value="value.reelId"
                    filterable
                    remote
                    :loading="reelLoading"
                    :options="reelOptions"
                    ...
                    @update:value="(val) => handleReelChange(val, value)"
                  />
                </div>

                <div class="detail-field">
                  <div class="detail-label required">SN/条码</div>
                  <n-input v-model:value="value.sn" placeholder="请输入或扫描唯一码" />
                </div>

                <div class="detail-field">
                  <div class="detail-label required">工艺版本</div>
                  <n-input v-model:value="value.craftVersion" placeholder="可选" />
                </div>

                <div class="detail-field">
                  <div class="detail-label required">层级</div>
                  <n-input-number v-model:value="value.layerIndex" :min="1" placeholder="层级" />
                </div>

                <div class="detail-field">
                  <div class="detail-label required">目标仓库</div>
                  <n-select
                    v-model:value="value.toWarehouseId"
                    filterable
                    remote
                    clearable
                    :loading="warehouseLoading"
                    ...
                    @update:value="(val) => handleToWarehouseChange(val, value)"
                  />
                </div>
              </div>
            </template>
