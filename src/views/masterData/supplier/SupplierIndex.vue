...
const permissionCodes = {
  create: 'MasterData.Supplier.Create',
  update: 'MasterData.Supplier.Update',
  delete: 'MasterData.Supplier.Delete',
} as const
...
function handleToolbarDelete() {
  ...
  const content = ids.length === 1 ? '确定要删除选中的供应商吗？' : `确定要删除选中的 ${ids.length} 条供应商吗？`
  dialog.warning({
    title: '提示',
    content,
    positiveText: '确定',
  ...
  <BaseCrudPage>
    <template #search>
      <n-form inline class="crud-search-form">
        <n-form-item>
          <n-input
            v-model:value="query.supplierCode"
