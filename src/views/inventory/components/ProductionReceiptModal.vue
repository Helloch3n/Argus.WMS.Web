...
<template>
  <n-modal v-model:show="visible" preset="card" title="生产完工入库" style="width: 760px">
    <n-form ref="formRef" :model="form" :rules="rules" label-width="140">
      <n-form-item label="来源工单" path="sourceWo">
        <n-input v-model:value="form.sourceWo" placeholder="请输入来源工单" />
      </n-form-item>

      <n-form-item label="批次号" path="batchNo">
        <n-input v-model:value="form.batchNo" placeholder="例如 BATCH-yyyymmdd" />
      </n-form-item>

      <n-form-item label="线盘" path="reelId">
        <n-select
          v-model:value="form.reelId"
          filterable
          remote
          :loading="reelLoading"
      ...
      <n-form-item label="长度（米）" path="quantity">
        <n-input-number v-model:value="form.quantity" :min="0" :step="1" placeholder="请输入长度" />
      </n-form-item>

      <n-form-item label="净重（kg）" path="weight">
        <n-input-number v-model:value="form.weight" :min="0" :step="0.1" placeholder="请输入净重" />
      </n-form-item>
    </n-form>
